...
- 元素出现动画使用页面组件自带的原生效果。
- 页面切换动画可以在预览控制面板里调整。
- 面向用户交付的 deck 默认不显示风格/主题切换选项;风格切换只保留在内部调试 demo 页面。用户明确要求保留主题切换,或用户提到/追问“线上网址可以更换模板风格、编辑 PPT”等在线编辑能力时,必须在 goal 顶层写 `preview: {"themeSwitcher": true}` 并重新渲染校验后再交付。峰峰已明确偏好后续使用本 skill 默认保留主题/模板风格切换入口,因此给他生成 PPT 时默认写入该字段,除非他明确要求关闭。
- 真正多模板切换:预览控制台的“预览风格”不是纯 CSS 皮肤切换;它只会在 HTML 中已经包含多个 `themePack` 对应 slides 时隐藏/显示不同主题包。给峰峰生成 HTML/线上预览 deck 时,不要只打开 `themeSwitcher=true`;默认同时为同一份内容生成至少 3 套可切换主题 slides:用户选定/默认主题 + `theme07` 冷白调研风 + `theme09` 深蓝杂志风。若用户选的主题已是其中之一,补足为 `theme01` 轻拟态风、`theme07`、`theme09` 三套。每套主题保持相同页数、相同信息结构和相同核心文案,但按该主题重新选唯一 layout 并按各 layout 的 `inspect:layout` 契约重填 props。若页数很长或某主题缺少可安全填充的同类布局,至少交付用户选定主题 + 1 套备选主题,并在最终说明中写明原因。
- 多主题 deck 组织:同一主题的一组 slides 连续排列,每个 slide 依靠 layout 推断/绑定对应 `themePack`;`themePack` 顶层仍写用户首选主题作为初始显示。每个主题包内部遵守“不重复 layout”和“只选一个封面页”的规则;不同主题之间可以使用各自的封面 layout。渲染后必须抽查 `preview-options.themePacks` 至少包含预期主题数,否则说明只是单主题预览,需要补齐多主题 slides 后重渲染。
- 开启 `themeSwitcher` 的副作用:渲染器会在 `preview-options` 中设置 `autosave:false`,避免多主题调试状态烧回 `index.html`。若用户需要在线编辑后持久保存,提醒其导出 PPTX/PDF/HTML 或另行保存定稿。
- 不手写自由 HTML slide;面向用户交付的每页必须写 `layout` + `props`。`role` 只允许在草稿阶段辅助选页,渲染前必须换成具体 `layout`。
- 每套主题的前 5 页 `themeXX_page001` 到 `themeXX_page005` 都是封面候选。单主题 deck 只能从前 5 页中选择 1 页作为封面,不要同时使用多个封面页;正文页从第 6 页以后选择。多主题切换 deck 则每个 `themePack` 各选 1 页封面,并保持各主题包页数一致。
- 同一套 PPT 中不要出现重复的页面组件:单主题内最终 `slides[].layout` 必须唯一。多主题 deck 按每个主题包分别保证 layout 唯一;选页时记录 `themePack + layout`,不同内容页要换同主题其它候选,不要通过改文案复用同一个 layout。
...
- HTML 交付:默认给用户的预览地址只给 `http://127.0.0.1:<port>/`(不给 https 或 .local 变体);本机 HTTP 可导出 HTML/PDF/PPTX,本地 HTML 或 `file://` 不能导出可编辑 PPTX。不要返回 `theme-preview`。在自带浏览器的 Agent APP(如 Codex)里生成时,提醒用户导出 PDF/PPTX 前把该地址在系统浏览器中打开。若用户明确要求“线上网址/公网链接/飞书可打开链接”,或指出 skill/产品说明承诺线上可编辑网址,不要只交付本机地址或 PPTX:应将当前预览服务通过现有公网反代/发布机制暴露为可访问 URL,验证公网 URL 返回 200 且包含本次 deck 标题后,把线上编辑 URL 放在最终回复最前面。
...
2. 确认 `themePack`。用户未指定时先询问风格;用户选定后生成 `randomSeed`,例如 `<主题>-<日期>-<3位随机词>`,保证随机选页可复现。峰峰场景默认启用多主题切换:确定主主题后同步确定备选主题集合(默认三套:主主题 + `theme07` + `theme09`,必要时用 `theme01` 补足),并在 goal 顶层写 `preview: {"themeSwitcher": true}`。
3. 判断图片意图:无图但需要视觉素材时先问是否预留图片槽;用户给本地素材先 `media:stage`;明确生图时用 image-gen。
4. 用 `layout:query` 选候选;对象/数组/count/图片 props 用 `inspect:layout` + `props:safe`。多主题 deck 要对每个目标 `themePack` 分别查询/inspect 对应 layout,不要把一个主题的 props 契约照搬到另一个主题。
...
12. 若用户需要线上编辑网址:将本地预览服务映射到已有公网 Nginx/发布路径,验证公网 URL `200 OK` 且页面包含本次 deck 标题;若同时需要风格切换,先设置 `preview.themeSwitcher=true`。峰峰场景默认要真正多模板切换,因此验证 `index.html` 内 `preview-options.themePacks` 至少包含预期主题数(默认 3 套,最低 2 套),否则补齐多主题 slides 并重新跑 `props:safe`、`validate:goal-spec`、`render:goal`、`validate:swiss`、`validate:goal-copy`。
13. 按交付格式回复:HTML 默认给 `http://127.0.0.1:<port>/`;有线上编辑诉求时优先给公网 URL;PPTX 调用 `/api/export-editable-pptx` 后只给 PPTX 文件路径或下载结果。

...
def local_image_as_data_url(value: str) -> str:
    """Encode a local image for the image-to-image endpoint without public hosting."""
    path = Path(value).expanduser().resolve()
    if not path.is_file():
        raise SystemExit(f"Input image file not found: {path}")
    mime, _ = mimetypes.guess_type(path.name)
    if mime not in {"image/png", "image/jpeg", "image/webp", "image/gif"}:
        raise SystemExit(f"Unsupported input image format: {path.name}; use PNG, JPEG, WEBP, or GIF")
    if path.stat().st_size > 10 * 1024 * 1024:
        raise SystemExit(f"Input image is too large ({path.stat().st_size} bytes); max is 10 MiB")
...
def cmd_image(args: argparse.Namespace) -> None:
    ...
    input_images = list(args.image or [])
    input_images.extend(local_image_as_data_url(path) for path in (args.image_file or []))
    if input_images:
        extra["image"] = input_images
    if extra:
        payload["extra_body"] = extra
    data = request_json("POST", "/v1/images/generations", payload)
    urls = extract_image_urls(data)
    output_result(
        "image-to-image" if input_images else "text-to-image",
        data,
        prompt_used=prompt,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    image.add_argument(
        "--image-file",
        action="append",
        help="Local input image file. Encoded in-memory as a data URL; it is not publicly hosted. Repeat for multiple images.",
    )
    image.add_argument(
        "--no-translate-prompt",
        action="store_true",

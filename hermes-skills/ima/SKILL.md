...
name: ima
description: |
  IMA OpenAPI 统一技能：笔记管理 + 知识库操作（上传文件、添加网页、搜索知识库等）。
  当用户提到知识库、笔记、备忘录、上传文件、搜索知识库、创建/编辑笔记时使用。
  触发关键词：知识库、笔记、记事、备忘录、ima、上传、文件库、资料库。
triggers:
  - 知识库
  - 笔记
  - 备忘录
...
> **Hermes 路径：** 调用脚本前设置：
> `SKILL_DIR="$HOME/.hermes/skills/productivity/ima"`
...
```bash
# Example usage in Hermes (credentials are read from ~/.config/ima automatically)
SKILL_DIR="$HOME/.hermes/skills/productivity/ima"
OPTS=$(printf '{"clientId":"%s","apiKey":"%s"}' "$IMA_OPENAPI_CLIENTID" "$IMA_OPENAPI_APIKEY")

# stdout 返回正常响应；stderr 返回结构化错误 {"code":-100|-200,"msg":"..."}
if ! resp=$(node "$SKILL_DIR/ima_api.cjs" "openapi/note/v1/list_note" '{"folder_id":"","sort_type":0,"cursor":"","limit":10}' "$OPTS" 2>/tmp/ima_err); then

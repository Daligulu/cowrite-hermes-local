...
## Hermes Routing Priority

- Use this Skill for explicit information graphics, visual summaries, dashboards, comparisons, timelines, and high-density information posters.
- If the user requests the established 峰峰 personal IP or 正文 IP 配图, route to `feng-ip` first; do not let this generic infographic trigger override the canonical personal-IP workflow.
- For ordinary multi-image article illustration rather than one information graphic, route to `baoyu-article-illustrator`.

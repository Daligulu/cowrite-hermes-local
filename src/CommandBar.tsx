...
import type { ActionConfig, ChannelConfig, CowriteTask, Page, TaskStatus, TaskPriority } from '../shared/types'
...
export function EditorCommandBar({ page, notify }: { page: Page; notify: (message: string) => void }) {
  ...
  const openTopicChoice = async (action: string, requirements?: string) => {
    setPendingChoice({ action, requirements })
    setPendingTopicReq((requirements ?? '').replace(/选题/g, '').trim())
    setPendingTopicType('article')
    try {
      const data = await api<{ config: { channels: ChannelConfig[] } }>('/api/channel-config')
  ...
  const submit = async (action?: string, requirements?: string) => {
    ...
    if (chosen === 'topic-collect' && !(req ?? '').includes('渠道')) {
      void openTopicChoice(chosen, req)
      return
    }
  ...
  const confirmTopic = async () => {
    ...
    const req = parts.filter(Boolean).join('；')
    const chosen = pendingChoice.action
    setPendingChoice(null)
    await doSubmit(chosen, req)
  }
  ...
  return (
    <div className="editor-command">
      ...
      {pendingChoice?.action === 'topic-collect' && (
        <div className="modal-mask" onClick={() => setPendingChoice(null)}>
          <div className="modal topic-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="modal-actions">
              <button onClick={() => setPendingChoice(null)}>取消</button>
              <button className="primary" onClick={() => void confirmTopic()} disabled={submitting}>开始收集</button>
            </div>
          </div>
        </div>
      )}

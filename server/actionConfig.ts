...
export const DEFAULT_ACTIONS: ActionConfig[] = [
  ...
  },
  {
    ...
    prompts: [
      { id: 'main', role: 'system', text: '你是公众号 9:16 知识讲解视频助手，按 broll-hyperframes skill 方案B 把当前页面文章做成竖屏短视频。第一步压缩：读 requirements/页面正文，压缩成 ≤60 秒中文叙述脚本（保留 hook/冲突/具体例子/结论/CTA，删除重复铺垫和元说明，短句，不编造工具/指标/结果；若原文已是很短的脚本可直接用），写到 /root/.cowrite/worker-assets/script.txt。第二步生成：解析 requirements 里的「讲解员比例：全身/半身/无」（未写默认全身）：全身 → 命令加 `--feng-mode full`；半身 → `--feng-mode half`；无 → 去掉 `--feng ...` 段不贴讲解员。运行 `P=/root/.hermes/hermes-agent/venv/bin/python3; $P /root/.hermes/scripts/cowrite-video.py --title "标题" --script-file /root/.cowrite/worker-assets/script.txt --feng /root/.cowrite/worker-assets/feng-guide.png --feng-mode <full|half> -o /root/.cowrite/worker-assets/gzh-video.mp4`（脚本负责 Edge TTS YunxiNeural+10% 男声配音 + Pillow 渲染逐句场景卡 + 右下角 Feng 讲解员（按所选比例：full=全身 / half=半身3-4身 / 无=不贴；透明底贴右下角、小比例、不遮字幕/进度/关键文本、带柔和投影）+ ffmpeg 拼接，输出 1080×1920 mp4）。第三步上传：用 mcp_cowrite_cowrite_upload_asset 上传 /root/.cowrite/worker-assets/gzh-video.mp4 到 Cowrite（返回 /assets/xxx.mp4）。第四步写回：把视频链接写回页面（附视频说明 + ffprobe 时长/分辨率）。红线：必须先压缩成短脚本再生成，不要直接朗诵长文；脚本里的字会逐字渲染到画面，中文必须正确；视频必须真实生成并用 ffprobe 验证（时长>0、1080×1920）后才写回，失败则如实 fail_task；不伪造视频/时长/分辨率。' },
    ],
    workflow: [],
  },
]

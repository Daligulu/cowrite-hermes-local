...
3. 调用 mcp_cowrite_cowrite_get_page 读取 pageId 的最新内容与 revision。根据 action、requirements、anchor 完成真实工作：
   ...
   - 通用资产链接规则（所有写回图片/文件的动作都必须遵守）：写回页面的图片一律用 mcp_cowrite_cowrite_upload_asset 返回的相对路径 `/assets/xxx.ext`，严禁自行拼成 `http://107.150.109.152/...` 或 `https://<隧道>/...` 等公网绝对 URL。前端 fixAssetLinks 会自动按当前入口把 `/assets/` 重写为可加载地址，若写入绝对 URL 会随隧道域名漂移/换入口而失效。
   - 配图增强验证（illustrate/feng-ip 完成前必须做，防「图存在但加载不出」假成功）：① 读回页面断言 `<img` 数量达标；② 每个图片 src 必须是 `/assets/` 相对路径（若是绝对 URL 视为污染，修正为相对路径再写回）；③ 至少一张图真实验证可加载（HEAD 请求 `/assets/<file>` 返回 HTTP 200 且 content-type 为 image/*）。三者任一不满足则调用 mcp_cowrite_cowrite_fail_task 写真实错误，不得标成功。
   - 排版保留配图（gzh-layout / wechat-layout 等排版动作，若是在配图动作之后执行）：排版时若页面 content 已含 `<img` 配图，必须原样保留这些配图及其相对 `/assets/` 路径，只对正文段落文本做排版（分段/标题/加粗/引用/字号行高），不得删除、覆盖或改写为绝对 URL；排版与配图互不冲突。
